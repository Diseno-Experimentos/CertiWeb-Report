...
1. Tests de User Entity
Dado que CertiWeb gestiona tres categorías de usuarios con distintos planes (Free, Mensual, Anual), la correcta inicialización de estas entidades asegura que el sistema pueda distinguir permisos y funcionalidades de acuerdo al plan adquirido. Por esta razón, hemos realizado pruebas exhaustivas en la clase User para prevenir errores en esta entidad fundamental que podrían permitir accesos no autorizados a funcionalidades premium o provocar inconsistencias en la facturación del sistema. Para validar esto, creamos una nueva instancia de User y confirmamos que todos sus atributos (nombre, correo electrónico, contraseña, plan) se inicialicen como cadenas vacías, garantizando que no existan valores predeterminados incorrectos que puedan ocasionar comportamientos inesperados en el sistema de permisos.
2. Tests de Reservation Entity
Las reservas constituyen el núcleo del negocio de CertiWeb, donde cada cita programada simboliza ingresos potenciales y un compromiso hacia el cliente. Por esta razón, llevamos a cabo pruebas exhaustivas que validan la estructura completa de la entidad Reservation, abarcando campos críticos como reservationName, imageUrl, brand, model, inspectionDateTime, price, status y userId. Aseguramos que cada reserva sea capaz de almacenar correctamente la información del vehículo (marca, modelo, imagen), los detalles de la cita (fecha, hora), el estado de la certificación y el precio del servicio. Esta verificación es fundamental, ya que la ausencia de cualquiera de estos campos podría ocasionar inspecciones no facturadas, técnicos no asignados o clientes no notificados.
3. Tests de Autenticación de Usuarios
La protección de los datos sensibles en CertiWeb se basa completamente en un sistema de autenticación sólido. Hemos creado pruebas que replican el proceso completo de inicio de sesión utilizando MockAdapter de axios, donde configuramos respuestas simuladas del servidor con datos de usuario que incluyen un token JWT. Comprobamos que, después de un inicio de sesión exitoso, el sistema almacene correctamente el token en localStorage, guarde la información del usuario (id, nombre, email, plan) en formato JSON, y que el método isAuthenticated() devuelva true. También evaluamos el caso de credenciales inválidas simulando una respuesta 401 del servidor, confirmando que el sistema no almacene ningún token y devuelva un mensaje de error adecuado al usuario.
4. Tests de Autenticación de Administradores
El acceso administrativo en CertiWeb es especialmente delicado, ya que permite la aprobación de certificaciones, la modificación de precios y la gestión completa de la base de datos de vehículos certificados.
Para salvaguardar estas funciones críticas, hemos implementado pruebas específicas que validan el endpoint de inicio de sesión administrativo, el cual está separado del inicio de sesión regular. Simulamos respuestas del servidor que incluyen el campo role: 'admin' y verificamos que el sistema almacene un adminToken adicional en localStorage, que es diferente del token de usuario regular.
Confirmamos que el método isAdmin() devuelve true únicamente cuando este token especial está presente, y que el objeto de respuesta contenga la propiedad isAdmin configurada en true, lo que permite que el frontend redirija correctamente al panel de administración.
5. Tests de Gestión de Sesiones y Logout
Teniendo en cuenta que los vendedores y concesionarias a menudo acceden a CertiWeb desde computadoras compartidas en oficinas, es esencial realizar una limpieza completa de las sesiones al cerrar sesión para garantizar la privacidad. Hemos desarrollado pruebas que primero llenan localStorage con authToken y currentUser, simulando así una sesión activa. Posteriormente, invocamos el método logout() y verificamos que ambos elementos sean eliminados por completo del almacenamiento local. También confirmamos la eliminación de adminToken y currentSession para asegurarnos de que no quede ningún rastro de la sesión anterior en el navegador. Esta prueba evita que el siguiente usuario en la misma computadora tenga acceso a certificaciones privadas o información confidencial del usuario anterior.
6. Tests de Creación de Certificaciones
Cada certificación vehicular en CertiWeb representa un ingreso directo que oscila entre S/. 1000-1800 y un compromiso legal respecto al estado del vehículo. Por esta razón, nuestras pruebas del servicio carService son exhaustivas y abarcan múltiples escenarios. Primero, evaluamos el caso exitoso en el que enviamos datos de un vehículo (marca: 'Ford', modelo: 'Focus') a través de una petición POST simulada que devuelve un código 201 junto con el objeto creado que incluye un ID. Verificamos que el servicio devuelva correctamente este objeto con todos sus campos. En segundo lugar, simulamos un error de red utilizando mock.networkError() y comprobamos que el sistema genere una excepción con un mensaje amigable como "No se pudo conectar al servidor" en lugar de fallar de manera silenciosa. Por último, probamos errores del servidor enviando una respuesta 400 con un mensaje específico y confirmamos que este mensaje se propague adecuadamente al frontend.
7. Tests de Sistema de Reservas
El flujo de reservas representa el embudo completo de conversión en CertiWeb, desde la solicitud inicial hasta la certificación final.
Implementamos un test integral que valida las cuatro operaciones CRUD esenciales en secuencia: primero, simulamos la creación de una reserva enviando datos de userId y verificando que retorne un objeto con ID asignado; segundo, probamos la obtención de todas las reservas confirmando que retorne un array; tercero, validamos la consulta de una reserva específica por ID verificando que retorne el objeto correcto; finalmente, probamos la actualización de estado de una reserva enviando un nuevo status y confirmando que el cambio se refleje en la respuesta.
Esta suite completa asegura que técnicos puedan ser asignados, clientes notificados, y certificaciones rastreadas sin interrupciones en ningún punto del flujo operativo.
8. Tests de Integración de Login End-to-End
A diferencia de las pruebas unitarias que evalúan funciones aisladas, en este caso validamos el componente completo de inicio de sesión montado con Vue Test Utils, simulando la interacción real de un usuario. Configuramos mocks de vue-router y vue-i18n para evitar dependencias externas, creamos un mock del authService que devuelve un inicio de sesión exitoso, luego interactuamos con el formulario estableciendo valores en los campos de correo electrónico y contraseña mediante setValue(), y finalmente disparamos el evento submit del formulario. Tras estas acciones, verificamos múltiples aspectos: que el servicio de autenticación fue invocado, que localStorage contenga un objeto currentSession con el userId correcto, y que el componente muestre el mensaje de éxito "¡Inicio de sesión exitoso! Redirigiendo...". Este enfoque de pruebas end-to-end asegura que todos los elementos funcionen en conjunto para ofrecer una experiencia fluida al usuario.
9. Tests de Integración del Catálogo de Vehículos
El catálogo CarList actúa como la principal vitrina donde los compradores buscan vehículos certificados y los vendedores administran su inventario. Para evaluar este componente esencial, implementamos una estrategia de simulación avanzada en la que sustituimos el módulo axios.config.js completo por una implementación que devuelve datos de prueba: un arreglo de vehículos con propiedades como id, marca, modelo, título y precio. Montamos el componente y esperamos que el ciclo de vida onMounted ejecute la carga de datos; luego, verificamos que la variable loading cambie a false y que el elemento .car-grid esté presente en el DOM. Además, probamos el filtrado por parámetros de búsqueda simulando useRoute de vue-router para devolver { query: { brand: 'ford' } }, y validamos que todos los vehículos en el arreglo resultante correspondan a la marca Ford, confirmando que los filtros funcionan adecuadamente para concesionarias que gestionan más de 50 vehículos simultáneamente.
10. Tests de Manejo de Errores en Credenciales
Los escenarios de error son tan cruciales como los casos exitosos, ya que impactan directamente en la experiencia del usuario y en la tasa de conversión. Hemos desarrollado dos pruebas específicas para el manejo de errores: en la primera, simulamos tanto el login como el loginAdmin para que retornen { success: false }, emulando credenciales incorrectas. Luego, ingresamos datos inválidos en el formulario y verificamos que ambos métodos de autenticación sean invocados de manera secuencial (primero el usuario regular y luego el admin), asegurándonos de que el errorMessage del componente contenga el texto "Credenciales incorrectas". En la segunda prueba, simulamos un error de red haciendo que el mock de login lance una excepción con new Error('Network'). Disparamos el submit del formulario y confirmamos que el mensaje mostrado sea "Error de conexión. Por favor, intenta de nuevo." Esta clara diferenciación entre tipos de error ayuda a los usuarios a entender si deben verificar sus credenciales o si hay un problema de conectividad.
11. Tests de Registro de Usuarios
El proceso de registro actúa como la entrada principal para nuevos clientes en CertiWeb, por lo que su correcto funcionamiento tiene un impacto directo en la adquisición de usuarios. Hemos implementado pruebas que simulan el endpoint POST /auth utilizando MockAdapter, enviando un objeto userData completo que incluye nombre, correo electrónico, contraseña y plan, y validamos que la respuesta contenga un token JWT. Verificamos que, tras un registro exitoso, el sistema almacene automáticamente tanto el authToken como el currentUser en localStorage, permitiendo que el usuario recién registrado esté inmediatamente autenticado sin necesidad de un segundo inicio de sesión. También confirmamos que el método retorne { success: true } junto con los datos del usuario creado, asegurando que el frontend pueda redirigir adecuadamente al dashboard y mostrar la información del perfil.
12. Tests de User Service
El userService funciona como un intermediario entre el frontend y el backend para realizar operaciones relacionadas con los datos de usuario. Evaluamos el método getCurrentUser() en tres situaciones diferentes: primero, con una sesión válida guardada en localStorage, confirmamos que el servicio realice una solicitud GET a /users/{userId} y devuelva los datos actualizados del usuario; segundo, examinamos el caso en el que no hay una sesión activa, asegurando que el método genere una excepción con el mensaje "No active session found", evitando así operaciones sin autenticación; tercero, simulamos un error del servidor (código 500) aunque haya una sesión en localStorage, y verificamos que el servicio devuelva la información almacenada de la sesión como alternativa, asegurando que la aplicación mantenga su funcionalidad básica incluso ante problemas de conectividad. También evaluamos el método logout() para comprobar que elimine currentSession del almacenamiento local.
13. Tests de Image Upload Service
El servicio ImgbbApiService es crucial para CertiWeb porque cada certificación requiere fotografías del vehículo inspeccionado como evidencia. Implementamos pruebas usando global.fetch mockeado ya que este servicio usa la API Fetch nativa en lugar de axios. En el caso exitoso, configuramos fetch para retornar { ok: true } con un JSON que contiene { data: { url: 'http://a' } }, luego llamamos uploadImage() pasando un Blob de prueba, y verificamos que el método retorne un objeto con la propiedad url correcta. Para el caso de error, mockeamos fetch para retornar { ok: false, statusText: 'ERR' } y confirmamos que el método lance una excepción con el mensaje "Error al subir imagen", permitiendo que el frontend muestre feedback apropiado cuando las fotografías no puedan cargarse, lo cual es crítico para no perder certificaciones por problemas técnicos de upload.
14. Tests de Validación de Respuestas del Servidor
CertiWeb debe manejar elegantemente las diferentes respuestas que puede enviar el backend, especialmente mensajes de error específicos que ayuden al usuario a corregir problemas. En nuestras pruebas del carService, simulamos una respuesta 400 del servidor que incluye un objeto JSON con la propiedad message: 'Bad request', y validamos que el servicio extraiga correctamente este mensaje y lo incluya en la excepción lanzada. También probamos respuestas con estructuras alternativas donde el mensaje puede estar en response.data.message o response.data.error, confirmando que el servicio tenga fallbacks apropiados. Adicionalmente, verificamos que cuando getAllCars() recibe un error 500, en lugar de lanzar una excepción que rompa la UI, retorne un array vacío, permitiendo que el catálogo muestre un estado vacío en lugar de una pantalla de error, manteniendo una mejor experiencia de usuario.
15. Tests de Consistencia de Datos en Autenticación
La sincronización entre diferentes áreas de almacenamiento es vital para evitar estados inconsistentes en la aplicación. Por ello, nuestras pruebas de autenticación no solo verifican que se guarde el token, sino que validan la estructura completa de los objetos almacenados. Confirmamos que después de un login, localStorage contenga tanto currentUser con las propiedades { id, name, email, plan } como currentSession con propiedades expandidas { userId, name, email, plan, isLoggedIn: true, isAdmin: false }. Verificamos que los IDs coincidan entre ambos objetos (currentUser.id === currentSession.userId) y que los campos duplicados (name, email) tengan valores idénticos. Esta redundancia controlada permite que diferentes partes de la aplicación accedan a información de usuario sin conflictos, mientras que nuestros tests aseguran que esta redundancia nunca resulte en inconsistencias de datos.

<img src="Images/reporte_pruebas_front.png"/>

Para el backend, implementamos pruebas exhaustivas para el backend utilizando NUnit, las cuales validan desde la lógica de certificación de vehículos (pruebas unitarias) hasta flujos completos de negocio end-to-end (pruebas de sistema). Estas pruebas se ejecutan automáticamente en el mismo job de build_and_test del pipeline CI/CD, garantizando así una validación integral del sistema que protege la credibilidad de las certificaciones emitidas.

1. Tests de Entidades del Dominio
1.1 Validación de Años de Vehículos (CarTests.cs)
CertiWeb certifica vehículos usados para reducir la desconfianza en el mercado automotriz de segunda mano. Si el sistema permitiera registrar vehículos con años inválidos (por ejemplo, un auto del año 1800), los compradores recibirían información incorrecta que destruiría la credibilidad de la plataforma. Este test garantiza que solo vehículos con años válidos (desde 1900 hasta la actualidad) puedan ser certificados, protegiendo así la reputación del servicio de certificación. Para validar esto, intentamos crear un vehículo con un año de 1800 y verificamos que el sistema rechace automáticamente esta información mediante una excepción, asegurando que ningún dato histórico incorrecto pueda comprometer la confianza de los usuarios en las certificaciones emitidas.
1.2 Validación de Precios Negativos (CarTests.cs)
El precio es una de las variables más importantes para los compradores al evaluar vehículos usados. Probamos la robustez del sistema intentando crear certificaciones con precios negativos (como -1000 soles), verificando que el sistema rechace inmediatamente estos valores anómalos. Esta validación es crítica porque CertiWeb se posiciona como una plataforma que ofrece información clara y verificada para ayudar a los compradores a tomar decisiones informadas. Si se permitieran precios inválidos, la transparencia prometida se vería comprometida, afectando directamente la velocidad de venta que los vendedores buscan obtener al utilizar el sello digital de certificación en sus publicaciones en línea.
1.3 Estandarización de Placas de Matrícula (LicensePlateTests.cs)
Las placas de matrícula son identificadores únicos que permiten construir el historial transparente del vehículo a lo largo del tiempo, uno de los diferenciadores clave de CertiWeb. Validamos que el sistema convierta automáticamente todas las placas a mayúsculas, independientemente de cómo sean ingresadas por el usuario (abc1234 se transforma en ABC1234). Esta estandarización previene que el mismo vehículo aparezca duplicado en el sistema con diferentes capitalizaciones, lo cual podría fragmentar su historial de inspecciones y certificaciones. Para revendedores y concesionarias que gestionan múltiples unidades, esta consistencia es fundamental para el seguimiento efectivo de sus certificaciones y para mantener la integridad del registro histórico que incrementa el valor de sus vehículos en el mercado.
1.4 Validación de Longitud de Placas (LicensePlateTests.cs)
Probamos que el sistema rechace placas de matrícula demasiado cortas (menos de 3 caracteres) o excesivamente largas (más de 15 caracteres), estableciendo límites que corresponden a los formatos reales de placas vehiculares en Latinoamérica. Esta validación protege contra errores de captura y posibles intentos de ingresar identificadores falsos. Cuando un inspector certificado evalúa un vehículo bajo el protocolo de más de cien puntos clave, la correcta identificación de la unidad mediante su placa es el primer paso para asociar el informe detallado generado con el vehículo correcto. Sin esta validación, podríamos generar certificados asociados a placas inválidas, haciendo imposible que compradores potenciales verifiquen posteriormente la certificación del vehículo que les interesa.
2. Tests de Integración con Base de Datos
2.1 Persistencia Completa de Certificaciones (CarIntegrationTests.cs)
El proceso de certificación técnica evalúa aspectos mecánicos, eléctricos, estructurales y estéticos del vehículo, generando un informe detallado acompañado de un sello digital que puede integrarse en publicaciones de venta. Verificamos que toda esta información compleja—incluyendo año, precio, placa y el PDF de certificación en formato Base64—se almacene correctamente en la base de datos MySQL y pueda recuperarse posteriormente sin pérdida de datos. Este test es esencial porque la propuesta de valor de CertiWeb depende completamente de que los compradores puedan acceder a certificaciones completas y confiables. Si falla el almacenamiento de cualquier componente del certificado, todo el proceso de inspección realizado por especialistas pierde su utilidad, afectando tanto a vendedores que pagaron por el servicio como a compradores que confían en la información para sus decisiones de compra.
2.2 Prevención de Certificaciones Duplicadas (CarIntegrationTests.cs)
Implementamos restricciones a nivel de base de datos que impiden registrar dos vehículos con la misma placa de matrícula. Para probar esto, intentamos guardar dos certificaciones diferentes para la misma placa y verificamos que el sistema rechace la segunda operación. Esta protección es fundamental para prevenir fraudes comunes en el mercado automotriz donde vendedores inescrupulosos intentan ocultar el historial real de un vehículo generando múltiples certificaciones contradictorias. Como CertiWeb busca profesionalizar el mercado de autos usados en Latinoamérica, mantener un registro único y transparente por vehículo es indispensable para construir la confianza que actualmente falta en las transacciones de segunda mano, permitiendo que tanto vendedores individuales como concesionarias incrementen la credibilidad de sus ofertas.
2.3 Relaciones entre Marcas y Vehículos (CarIntegrationTests.cs)
Validamos que las relaciones entre marcas (Toyota, Honda, etc.) y vehículos certificados se mantengan correctas en la base de datos, verificando que al recuperar un vehículo también podamos acceder automáticamente a su marca asociada. Este test garantiza que las herramientas de gestión ofrecidas a negocios automotrices funcionen correctamente, especialmente las búsquedas y filtros por marca que permiten a revendedores y concesionarias organizar su inventario certificado. Para compradores, la capacidad de buscar vehículos certificados por marca específica es una funcionalidad esperada que facilita encontrar opciones que se ajusten a sus preferencias. Probamos creando vehículos asociados a diferentes marcas y luego ejecutando consultas filtradas, asegurando que los resultados sean precisos y que el sistema pueda escalar eficientemente cuando concesionarias carguen masivamente cientos de vehículos de múltiples marcas.
2.4 Consultas por Rango de Precios (CarIntegrationTests.cs)
Creamos vehículos certificados con diferentes precios (desde 15,000 hasta 50,000 soles) y luego ejecutamos búsquedas filtrando por rangos específicos, verificando que el sistema retorne únicamente los vehículos que cumplan los criterios establecidos. Esta funcionalidad es crítica para compradores que tienen presupuestos definidos y necesitan filtrar rápidamente las opciones disponibles dentro de su capacidad de pago. CertiWeb no solo certifica vehículos sino que también facilita su descubrimiento en el mercado, y la capacidad de filtrar por precio permite que el sello digital de certificación agregue valor tanto a vehículos económicos como de lujo. Para revendedores, esto significa que sus unidades certificadas serán más fáciles de encontrar por el público objetivo correcto, incrementando efectivamente la velocidad de venta que buscan al invertir en el servicio de certificación.
2.5 Unicidad de IDs de Reserva (CarIntegrationTests.cs)
Cada vehículo en CertiWeb tiene asociado un ID de reserva original que vincula la certificación con el sistema de gestión interno. Probamos que el sistema impida registrar dos vehículos con el mismo ID de reserva, intentando crear certificaciones duplicadas y verificando que la base de datos rechace la operación. Esta restricción protege la integridad operativa de la plataforma, especialmente importante cuando concesionarias y revendedores utilizan la carga masiva de vehículos, donde errores en archivos CSV o integraciones API podrían causar duplicaciones accidentales. Si no validáramos esta unicidad, podríamos tener inconsistencias entre las certificaciones emitidas y los registros internos de gestión, complicando el seguimiento de certificaciones y el soporte especializado que CertiWeb ofrece a negocios automotrices.
3. Tests de Flujos de Negocio Completos
3.1 Flujo Completo de Certificación End-to-End (BusinessFlowSystemTests.cs)
Simulamos el recorrido completo que realiza un vehículo en la plataforma: primero verificamos que las marcas estén disponibles para selección, luego un revendedor registra un Toyota Camry 2023 con todos sus datos incluyendo el PDF de certificación, un inspector recupera la información para validarla, y finalmente un comprador potencial busca el vehículo tanto en listados generales como en la base de datos para verificar su certificación. Este test valida la cadena de valor completa de CertiWeb, desde el registro inicial hasta la consulta por compradores interesados. Si cualquier paso de este flujo fallara—sea el registro, la recuperación de datos o la búsqueda—toda la promesa de valor de la plataforma se vería comprometida. Es el test más importante desde la perspectiva del negocio porque valida que todos los actores del ecosistema (vendedores, inspectores certificados y compradores) puedan cumplir sus objetivos usando la plataforma, asegurando que la información clara y verificada llegue efectivamente a quien la necesita.
3.2 Operaciones Concurrentes de Múltiples Usuarios (BusinessFlowSystemTests.cs)
Lanzamos simultáneamente 10 registros de vehículos diferentes, simulando revendedores trabajando al mismo tiempo durante horas pico, y verificamos que todas las operaciones se completen exitosamente sin pérdida de datos ni conflictos. Este escenario refleja la realidad operativa cuando CertiWeb escala su servicio a múltiples concesionarias grandes que necesitan certificar docenas de vehículos diariamente. Probamos que cada vehículo mantenga su placa única, su ID de reserva único y que todos los registros queden correctamente almacenados y sean recuperables posteriormente. Para negocios automotrices que dependen de procesar grandes volúmenes de inventario, la capacidad de la plataforma de manejar carga masiva sin errores es determinante para adoptar el servicio. Este test garantiza que CertiWeb pueda crecer desde vendedores individuales hasta concesionarias con cientos de unidades, manteniendo la confiabilidad del sistema en todo momento.
3.3 Manejo de Errores y Recuperación (BusinessFlowSystemTests.cs)
Intentamos deliberadamente crear una certificación con múltiples datos inválidos: título vacío, email mal formado, año imposible (1800), precio negativo y placa demasiado corta. Verificamos que el sistema rechace esta solicitud con un código de error apropiado (BadRequest), luego corregimos todos los datos y confirmamos que la certificación se cree exitosamente. Este test valida la resiliencia del sistema ante errores humanos comunes durante la captura de datos, especialmente relevante cuando inspectores en campo ingresan información de vehículos en dispositivos móviles o cuando revendedores cargan archivos con errores de formato. La capacidad del sistema de rechazar datos incorrectos sin colapsar y permitir reintentos exitosos asegura que errores operacionales no impidan completar certificaciones legítimas. Para CertiWeb, mantener la estabilidad del sistema incluso ante errores de usuario es crítico para ofrecer el soporte especializado prometido a negocios automotrices y mantener la productividad de los inspectores certificados en campo.
3.4 Descubrimiento de Vehículos por Compradores (BusinessFlowSystemTests.cs)
Creamos un inventario diverso de tres vehículos certificados: un Toyota Corolla económico a 25,000 soles, un Honda Civic deportivo a 32,000 soles y un Toyota Camry de lujo a 45,000 soles, cada uno con descripciones y características distintas. Luego simulamos a un comprador explorando la plataforma: primero consulta todas las marcas disponibles, después revisa todos los vehículos certificados, examina en detalle cada uno por su ID específico, y finalmente verifica que existan opciones en diferentes rangos de precio. Este flujo representa el journey típico de un comprador que busca reducir la incertidumbre al comprar un auto usado mediante las certificaciones de CertiWeb. Validamos que toda la información esté accesible y consistente en cada paso, porque si un comprador no puede encontrar fácilmente vehículos certificados o acceder a sus detalles completos, el valor del sello digital de certificación que los vendedores pagaron para obtener mejor posicionamiento en el mercado se pierde completamente.
3.5 Prevención de Fraude en Certificaciones (CarsControllerSystemTests.cs)
Intentamos registrar dos vehículos diferentes usando la misma placa de matrícula y verificamos que el sistema acepte el primero pero rechace inmediatamente el segundo con un error de validación. Este comportamiento protege contra un fraude común en el mercado automotriz donde se intenta certificar el mismo vehículo múltiples veces con información contradictoria para ocultar problemas o accidentes previos. La detección instantánea de duplicados a nivel de API REST asegura que ni siquiera lleguen a procesarse solicitudes fraudulentas, protegiendo la integridad del historial de certificaciones que es fundamental para el modelo de negocio de CertiWeb. Esta prueba valida que las reglas de negocio para prevenir fraude están implementadas correctamente en todos los puntos de entrada del sistema, no solo en la base de datos, sino también en la lógica de aplicación que enfrentan directamente revendedores, concesionarias e inspectores certificados.
3.6 Creación y Recuperación Individual de Vehículos (CarsControllerSystemTests.cs)
Registramos un vehículo con todos sus datos completos de certificación y luego intentamos recuperarlo por su ID específico, verificando que toda la información se mantenga intacta: título, propietario, email, año, modelo, descripción, precio, placa y certificación PDF. También probamos que al buscar un ID inexistente el sistema responda apropiadamente con un error NotFound en lugar de fallar inesperadamente. Esta validación cubre el caso de uso más frecuente en CertiWeb: un comprador interesado encuentra una publicación de venta con el sello digital, hace clic para ver la certificación completa, y necesita acceder instantáneamente a todos los detalles del informe técnico generado por especialistas. Si esta operación básica fallara o retornara datos incompletos, todo el proceso de evaluación de más de cien puntos clave perdería su utilidad práctica, destruyendo la confianza tanto de compradores en la información como de vendedores en el retorno de inversión del servicio de certificación.
4. Tests de Comportamiento del Repositorio
4.1 Búsqueda de Vehículos por Placa (CarRepositoryBehaviorTests.cs)
Probamos la búsqueda de vehículos utilizando diferentes formatos de placas (ABC-456, XYZ-456, 123-456) para verificar que el repositorio pueda localizar correctamente cualquier vehículo certificado sin importar la variación en el formato de su placa. Esta funcionalidad es el punto de entrada principal para compradores que ven un vehículo de interés en la calle o en una publicación y quieren verificar si cuenta con certificación de CertiWeb. La búsqueda por placa debe funcionar de manera rápida y precisa porque representa el momento crítico donde un comprador potencial decide si confiar en la información del vendedor o no. Para CertiWeb, este test valida que el valor agregado del sello digital de certificación sea realmente accesible y útil en el momento de la decisión de compra, cumpliendo la promesa de reducir la desconfianza mediante información verificada y fácilmente consultable.
4.2 Consistencia en Consultas Repetidas (CarRepositoryBehaviorTests.cs)
Realizamos dos consultas consecutivas del mismo vehículo por su ID y verificamos que ambas retornen exactamente la misma información sin variaciones ni inconsistencias. Aunque parece simple, esta prueba es fundamental para garantizar que el historial de certificaciones—uno de los diferenciadores clave de CertiWeb—sea confiable a lo largo del tiempo. Los compradores necesitan poder consultar un certificado múltiples veces durante su proceso de decisión, compartirlo con mecánicos de confianza o compararlo con otros vehículos, y en cada acceso la información debe ser idéntica. Para revendedores y concesionarias, la consistencia de datos permite referencias cruzadas entre su inventario y el sistema de CertiWeb sin preocuparse por discrepancias que podrían confundir a clientes o complicar procesos de venta. Este test valida la estabilidad y confiabilidad del repositorio de datos que sustenta toda la plataforma.
4.3 Filtrado de Vehículos por Marca (CarRepositoryBehaviorTests.cs)
Creamos vehículos de diferentes marcas (Toyota y Honda) y ejecutamos consultas filtradas para verificar que el repositorio retorne únicamente los vehículos de la marca solicitada, sin mezclar resultados. Este test valida las herramientas de gestión que CertiWeb ofrece a negocios automotrices, permitiéndoles organizar y hacer seguimiento de certificaciones por marca dentro de su inventario diversificado. Para concesionarias que manejan múltiples marcas simultáneamente, poder filtrar eficientemente sus vehículos certificados les permite segmentar sus campañas de marketing, analizar qué marcas se venden más rápido con certificación, y gestionar el inventario de manera más estratégica. El filtrado preciso por marca también facilita que compradores enfocados en marcas específicas encuentren opciones relevantes, incrementando la efectividad del sello digital de certificación para posicionar vehículos en el mercado.
4.4 Validación de Existencia de Vehículos (CarRepositoryBehaviorTests.cs)
Consultamos el repositorio buscando un vehículo que existe y otro que no existe, verificando que el sistema responda apropiadamente en cada caso: retornando datos completos para el vehículo válido y null para el inexistente, sin generar errores inesperados. Este comportamiento predecible es esencial para las integraciones que revendedores y concesionarias realizan entre CertiWeb y sus sistemas de inventario internos. Si el sistema fallara al consultar IDs inexistentes—algo común en ambientes de producción donde referencias pueden quedar desactualizadas—las integraciones automáticas colapsarían, requiriendo intervención manual y reduciendo el valor de las herramientas de gestión que CertiWeb ofrece. Este test asegura que el sistema sea robusto ante situaciones del mundo real donde no toda consulta necesariamente corresponde a un vehículo certificado existente.
4.5 Operaciones de Actualización de Vehículos (CarRepositoryBehaviorTests.cs)
Probamos que el repositorio pueda actualizar correctamente la información de vehículos ya certificados, simulando escenarios donde inspectores necesitan corregir datos después de la primera captura o donde vendedores actualizan precios de vehículos certificados según evoluciona el mercado. Verificamos que las actualizaciones se apliquen correctamente y que el sistema mantenga la integridad referencial con otras entidades relacionadas como las marcas. Para CertiWeb, permitir actualizaciones controladas es importante porque el mercado automotriz es dinámico: los precios fluctúan, los propietarios cambian, y ocasionalmente se descubren errores en certificaciones que deben corregirse. Sin embargo, estas actualizaciones deben hacerse de manera que preserven la trazabilidad del historial de certificaciones, asegurando que cualquier modificación quede registrada y no comprometa la confianza de compradores en la información histórica del vehículo.
4.6 Operaciones de Eliminación de Registros (CarRepositoryBehaviorTests.cs)
Validamos que el repositorio pueda eliminar vehículos del sistema cuando sea necesario, verificando que después de la eliminación el vehículo ya no sea recuperable mediante consultas normales. Esta funcionalidad es necesaria para casos excepcionales como certificaciones solicitadas por error, vehículos que resultaron ser robados o clonados, o cuando un vendedor solicita explícitamente la eliminación de su certificación. Aunque parece contradictorio con el objetivo de mantener un historial permanente, en la práctica es un requisito legal y ético poder eliminar información bajo circunstancias específicas. Para CertiWeb, este test asegura que la plataforma cumpla con regulaciones de protección de datos y pueda responder apropiadamente a situaciones extraordinarias sin comprometer la integridad general del sistema, manteniendo así la confianza tanto de usuarios como de autoridades en la responsabilidad con la que se maneja la información sensible de certificaciones automotrices.

<img src="Images/report_coverage_backend.png"/>
